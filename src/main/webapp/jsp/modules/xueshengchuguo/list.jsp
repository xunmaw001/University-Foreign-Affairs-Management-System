...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">学生出国管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>学生出国列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">学生出国列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										出国原因
									</label>
									<input type="text" id="chuguoyuanyinSearch" class="form-control form-control-sm" placeholder="请输入出国原因" aria-controls="tableId">
									<label>
										学生姓名
									</label>
									<input type="text" id="xueshengxingmingSearch" class="form-control form-control-sm" placeholder="请输入学生姓名" aria-controls="tableId">
                                    <label>
                                        是否通过
                                    </label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('liuxueguojia')">留学国家<i id="liuxueguojiaIcon" class="fa fa-sort"></i></th>
											<th>申请文件</th>
											...
											<th onclick="sort('yuanxi')">院系<i id="yuanxiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#chuguoyuanyinSearch').val() != null && $('#chuguoyuanyinSearch').val() != ''){
				searchForm.chuguoyuanyin ="%" +  $('#chuguoyuanyinSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("xueshengchuguo/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				chuguoyuanyin : searchForm.chuguoyuanyin,	
		...
		function setDataRow(item,number){
			...
			var liuxueguojiaCell  = document.createElement('td');

						 liuxueguojiaCell.innerHTML = item.liuxueguojia;

			row.appendChild(liuxueguojiaCell);
			var shenqingwenjianCell  = document.createElement('td');

						  var shenqingwenjianFile = document.createElement('button');
			  shenqingwenjianFile.setAttribute('class','btn-table')
			  shenqingwenjianFile.innerHTML = '下载';
			  var shenqingwenjianFileAttr = "download('" +baseUrl+ item.shenqingwenjian + "')"
			  shenqingwenjianFile.setAttribute('onclick',shenqingwenjianFileAttr);
        	          shenqingwenjianCell.appendChild(shenqingwenjianFile);


			row.appendChild(shenqingwenjianCell);
			...
			var yuanxiCell  = document.createElement('td');

						 yuanxiCell.innerHTML = item.yuanxi;

			row.appendChild(yuanxiCell);
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "学分申请"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'xuefenheding','是','','')");
			if(crossBtnControl('学分申请') == true){
		...
		function sh(){
			...
			httpJson("xueshengchuguo/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("xueshengchuguo/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"xueshengchuguo");
