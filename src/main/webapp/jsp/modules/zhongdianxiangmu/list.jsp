...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">重点项目管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>重点项目列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">重点项目列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										项目编号
									</label>
									<input type="text" id="xiangmubianhaoSearch" class="form-control form-control-sm" placeholder="请输入项目编号" aria-controls="tableId">
									<label>
										项目名称
									</label>
									<input type="text" id="xiangmumingchengSearch" class="form-control form-control-sm" placeholder="请输入项目名称" aria-controls="tableId">
									<label>
										教师姓名
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('xiangmubianhao')">项目编号<i id="xiangmubianhaoIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xiangmumingcheng')">项目名称<i id="xiangmumingchengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('xiangmujianjie')">项目简介<i id="xiangmujianjieIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#xiangmubianhaoSearch').val() != null && $('#xiangmubianhaoSearch').val() != ''){
				searchForm.xiangmubianhao ="%" +  $('#xiangmubianhaoSearch').val() + "%";
			}
			
			if($('#xiangmumingchengSearch').val() != null && $('#xiangmumingchengSearch').val() != ''){
				searchForm.xiangmumingcheng ="%" +  $('#xiangmumingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("zhongdianxiangmu/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				xiangmubianhao : searchForm.xiangmubianhao,	
				xiangmumingcheng : searchForm.xiangmumingcheng,	
		...
		function setDataRow(item,number){
			...
			var xiangmubianhaoCell  = document.createElement('td');

						 xiangmubianhaoCell.innerHTML = item.xiangmubianhao;

			row.appendChild(xiangmubianhaoCell);
			var xiangmumingchengCell  = document.createElement('td');

						 xiangmumingchengCell.innerHTML = item.xiangmumingcheng;

			row.appendChild(xiangmumingchengCell);
			var xiangmujianjieCell  = document.createElement('td');

						 xiangmujianjieCell.innerHTML = item.xiangmujianjie;

			row.appendChild(xiangmujianjieCell);
			var xiangmupingguCell  = document.createElement('td');

			
			var xiangmuxiangqingCell  = document.createElement('td');
			...
			var crossBtn0 = document.createElement('button');
            		crossBtn0.innerHTML = "项目总结"
			crossBtn0.setAttribute("type","button");
			crossBtn0.setAttribute("class","btn btn-primary btn-sm");
			crossBtn0.setAttribute("onclick","crossTable(" + JSON.stringify(item) + ",'xiangmuzongjie','是','','')");
			if(crossBtnControl('项目总结') == true){
		...
		function sh(){
			...
			httpJson("zhongdianxiangmu/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("zhongdianxiangmu/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"zhongdianxiangmu");
